...
#global pubkey, privkey, kill_thread
...
def client_server_flow():
    ...
    def display(message, time):
        global kill_thread
        kill_thread = False
        
        print(message)
        
        os.system(f"sudo ./rpi/display-scrolling -f ./rpi/fonts/8x13.bdf --led-gpio-mapping=adafruit-hat --led-cols=64 -s {time} {message}")
    ...
    def handle(client, enc):
        global kill_thread, thread_started
        while True:
            try:
                ...
                if not thread_started: # if thread has not been started
                    print("displaeyd")
                else:
                    print("displayed 2")
                    kill_thread = True
                    
                thread_display = threading.Thread(target=display, args=(message, 0.5)) # scroll time in seconds
